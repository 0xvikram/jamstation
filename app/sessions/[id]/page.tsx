import { ArrowLeft, Users, Mic, Video, VideoOff, MicOff, MessageSquare, Hand, Radio, PlaySquare, Settings, DollarSign, Send, MapPin, Share2 } from "lucide-react";
import Link from "next/link";
import { Mic2, Star } from "lucide-react";

export default function SessionStage({ params }: { params: { id: string } }) {
    // Mock data
    const isHost = false; // toggle this visually for now
    const audience = [
        { name: "Alex R.", waiting: true },
        { name: "Jordan K.", waiting: false },
        { name: "Chris M.", waiting: false },
        { name: "Sam L.", waiting: true },
        { name: "Taylor B.", waiting: false },
    ];
        ...
        <div className="min-h-screen bg-black text-slate-50 font-sans flex flex-col">
            ...
            <nav className="h-16 border-b border-white/5 px-6 flex items-center justify-between bg-black/50 sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard" className="text-slate-400 hover:text-white transition-colors p-2 rounded-full hover:bg-white/5">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-slate-400 flex items-center gap-2 mt-0.5">
                            Hosted by <span className="font-medium text-slate-300">Miles D.</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button className="px-4 py-2 rounded-lg font-medium text-sm border border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-colors">
                        Leave Session
                    </button>
                </div>
            </nav>
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden max-w-[1600px] w-full mx-auto">
                ...
                <main className="flex-1 flex flex-col p-4">
                    ...
                    <div className="flex-1 rounded-2xl bg-slate-900 border border-white/5 relative overflow-hidden group shadow-2xl flex flex-col items-center justify-center">
                        ...
                        {/* Mock Avatar because video API is off */}
                        ...
                        <div className="absolute bottom-6 left-6 z-20 flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-indigo-500 flex items-center justify-center text-lg font-bold">M</div>
                            <div>
                                <div className="font-bold text-lg text-white drop-shadow-md">Miles D.</div>
                                <div className="text-sm font-medium text-indigo-300 drop-shadow-md">Host</div>
                            </div>
                        </div>

                        {/* Controls (Host or Co-jamming) */}
                ...
                <aside className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-white/5 bg-slate-950 flex flex-col pt-4 pb-4 px-4 h-[400px] lg:h-auto shrink-0 z-30 shadow-[-10px_0_30px_rgba(0,0,0,0.5)]">
                    ...
                    <div className="mb-6 space-y-3">
                        <button className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-400 hover:to-indigo-500 text-white font-bold text-sm shadow-lg shadow-indigo-500/20 transition-all flex items-center justify-center gap-2">
                            <Hand className="w-4 h-4" /> Request to Join Stage
                        </button>
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        ...
                        <div className="flex gap-4 border-b border-white/10 pb-2 mb-4">
                            ...
                            <button className="text-sm font-semibold text-slate-500 hover:text-slate-300 pb-2 -mb-2.5">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                            <div className="text-xs text-center text-slate-500 font-medium py-2">Welcome to the session!</div>
                            {/* Mock Messages */}
                            <div>
                                <span className="font-bold text-indigo-400 text-sm mr-2">Jordan K.</span>
                                <span className="text-sm text-slate-200">That solo was crazy! 🔥</span>
                            </div>
                            <div>
                                <span className="font-bold text-pink-400 text-sm mr-2">Alex R.</span>
                                <span className="text-sm text-slate-200">What scale was that?</span>
                            </div>
                            <div>
                                <span className="font-bold text-slate-300 text-sm flex items-center gap-1 mb-1">
                                    <DollarSign className="w-3 h-3 text-emerald-400" />
                                    <span className="text-emerald-400">Sam L. tipped $5!</span>
                                </span>
                                <span className="text-sm text-slate-400 italic">"Keep it playing!"</span>
                            </div>
                        </div>
